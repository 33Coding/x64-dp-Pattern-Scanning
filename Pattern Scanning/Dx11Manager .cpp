...
class Dx11Manager : public Singleton<Dx11Manager>
{
	friend class Singleton<Dx11Manager>;
...
public:
	...
	bool InitializeHook();
	bool HookPresent(IDXGISwapChain* swapChain); //Hook Present with the swapChain ptr        

	void SetPresentCallBack(Callback_t val);
	Callback_t PresentCallBack() const;

	void UnHook();

	float GetWidth() const; //Return the Width of the window
	float GetHeight() const; //Return the Height of the window
...
private:
	...
	DWORD_PTR* m_pSwapChainVtable = nullptr;

	bool CreateDeviceAndSwapChain();
	void ReleaseDevices();

	bool HookDynamicallyPresent();

	Callback_t m_PresentCallBack;
};
